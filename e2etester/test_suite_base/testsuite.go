...
func CreateTestSuite(tOptions *executor.TestOptions, project string, regex *regexp.Regexp) (*TestSuite, error) {
	fmt.Printf("creating test suite: %s\n", tOptions.Name)
	var t TestSuite

	t.Name = tOptions.Name
	if project == "" {
		return nil, errors.New("no project provided")
	}
	t.Project = project

	if len(tOptions.GetImages()) == 0 {
		return nil, errors.New("no images provided")
	}
	t.Images = tOptions.GetImages()
	...
	workflows, err := workflow.CreateTestWorkflows(tOptions.GetTestHandle(), tOptions.GetImages(), regex)
	if err != nil {
		return nil, err
	}
...
//
func execute(testHandle interface{}) {
	err := e2etester.RunLocalTest(testHandle)
	if err != nil {
...
func Start(testHandle *executor.TestOptions, wg *sync.WaitGroup) {
	defer wg.Done()
	if IsRunningOnGCE() {
		execute(testHandle.GetTestHandle())
		return
	}
	Prepare(testHandle)
}
